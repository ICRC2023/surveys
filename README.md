...
- uvを使ってPythonの仮想環境を構築します。
- 任意の作業用ディレクトリに移動して、リポジトリをクローンする（``git clone``）
- 必要なパッケージをローカルな仮想環境（``.venv``）にインストールする（``uv sync --all-groups``）
...
- ``30_comments.ipynb``
- ``q02_gender.ipynb``
- ``q19_interest.ipynb``（など）

Marimo への移行も進めています。移行後は ``task nb:edit``、``task nb:run`` で起動できます。
